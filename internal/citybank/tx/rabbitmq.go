...
const (
	completedTransfersTopic = "citybank.v1.completed_transfers"
	transfersQueue          = "citybank.transfers"
)
...
func (s *Worker) ProcessTransfer(d amqp.Delivery) {
	...
	ctx, span := otel.Start(ctx, otel.NoTracer, fmt.Sprintf("%s.TxWorker/ProcessTransfer", app))
	defer span.End()
	...
	g.Go(func() error {
		a, err := s.AccountGet(ctx, tx.SrcAccount.Type, tx.SrcAccount.Number)
		if err != nil {
	...
	g.Go(func() error {
		a, err := s.AccountGet(ctx, tx.DstAccount.Type, tx.DstAccount.Number)
		if err != nil {
